...
def _softplus_inverse(x):
  ...
  return tf.log(tf.math.expm1(x)) # not defined for x < 1! But it does not matter because it's only called on the value 1 (at least in this script)
...
def make_encoder(activation, latent_size, base_depth):
  ...
  encoder_net = tf.keras.Sequential([
      conv(base_depth, 5, 1), # filters (number of filters) = base_depth, kernel_size (size of convolution window) = 5, stride (step in rolling window) = 1
      conv(base_depth, 5, 2),
      conv(2 * base_depth, 5, 1),
      conv(2 * base_depth, 5, 2),
      conv(4 * latent_size, 7, padding="VALID"), # default stride value is 1, so valid padding implies a shrinkage of the output size
      tf.keras.layers.Flatten(),
      tf.keras.layers.Dense(2 * latent_size, activation=None, kernel_initializer=tf.initializers.glorot_uniform(seed=0)), # twice the latent size probably because one parameter is for location and the other for scale of the normal distribution
  ])

  def encoder(images):
    # TODO: why this next operation?
    # TODO: isn't it bad practice to re-use function arguments?
    images = 2 * tf.cast(images, dtype=tf.float32) - 1
    net = encoder_net(images)
    return tfd.MultivariateNormalDiag(
        loc=net[..., :latent_size], # TODO: what are the other dimensions in the encoder, other than the last one?
        scale_diag=tf.nn.softplus(net[..., latent_size:] +
                                  _softplus_inverse(1.0)), # TODO: I don't see the purpose of this operation adding the softplus inverse of 1
        name="code")
...
def make_decoder(activation, latent_size, output_shape, base_depth):
  ...
  # TODO: This network does not seem the inverse of the encoder network, e.g.
  # the first layer has 2 * base_depth when the last layer of the encoder uses 4 * latent_size
  # etc.
  # TODO: and why the last convolution layer?
  ...
  def decoder(codes):
    ...
    codes = tf.reshape(codes, (-1, 1, 1, latent_size)) # TODO: why reshape?
...
def pack_images(images, rows, cols):
  ...
  # TODO: aren't the next four lines useless?
  shape = tf.shape(images)
  width = shape[-3]
  height = shape[-2]
  depth = shape[-1]
  images = tf.reshape(images, (-1, width, height, depth))
  # TODO: I don't understand the rest of this function. Does it rearrange
  # the images so one image displays many of them?
...
def model_fn(features, labels, mode, params, config):
  ...
  # .sample() on the encoder function is a tfd.MultivariateNormalDiag
  # which inherits indirectly from class Distribution, which has a .sample() method
  # defined on line 675 of:
  #https://github.com/tensorflow/probability/blob/master/tensorflow_probability/python/distributions/distribution.py
  approx_posterior_sample = approx_posterior.sample(params["n_samples"], seed = 0)
  ...
  distortion = -decoder_likelihood.log_prob(features) # This is the sample analog of  -E_{Z~q(Z|x)}[log p(x|Z)] in the module docstring.
  ...
  if params["analytic_kl"]:
    ...
    rate = (approx_posterior.log_prob(approx_posterior_sample)
            - latent_prior.log_prob(approx_posterior_sample)) # This is sample analog of the term - KL[q(Z|x) || p(Z)] in the module docstring
  avg_rate = tf.reduce_mean(rate)
  tf.summary.scalar("rate", avg_rate)

  elbo_local = -(rate + distortion) # TODO: I believe this should be -rate + distortion

  elbo = tf.reduce_mean(elbo_local)
  loss = -elbo
  tf.summary.scalar("elbo", elbo)

  # TODO: why is this called "importance weighted" if it does not use
...
def static_mnist_dataset(directory, split_name):
  ...
  str_to_arr = lambda string: np.array([c == b"1" for c in string.split()]) # this binary 1 because the file was binarized, as in FILE_TEMPLATE = "binarized_mnist_..."

  def _parser(s):
    booltensor = tf.py_func(str_to_arr, [s], tf.bool)
    reshaped = tf.reshape(booltensor, [28, 28, 1])
    return tf.to_float(reshaped), tf.constant(0, tf.int32) # TODO: Why are all labels 0? Is it because we are learning to generate images and not to classify them?
...
def build_fake_input_fns(batch_size):
  ...
  def train_input_fn():
    dataset = tf.data.Dataset.from_tensor_slices(
        random_sample).map(lambda row: (row, 0)).batch(batch_size).repeat() # 0 = always assign label 0
...
def build_input_fns(data_dir, batch_size):
  ...
  def train_input_fn():
    ...
    dataset = dataset.shuffle(50000, seed=0).repeat().batch(batch_size) # How can TensorFlow deal with infinite datasets?
    # For compatibility with TF2 and with the Distribute strategy,
    # we need to return a dataset and not `eval_dataset.make_one_shot_iterator().get_next()`
    return dataset
...
def main(argv):
  ...
  with tf.Session(config=tf.ConfigProto(inter_op_parallelism_threads=1,
      ...
      for _ in range(FLAGS.max_steps // FLAGS.viz_steps):
          ...
          current_steps += FLAGS.viz_steps
          eval_results = estimator.evaluate(eval_input_fn)

          step_str = "Results at %d :%s" % (current_steps, eval_results)
          duration_str = "Seconds running model = %1.12f" % (end-start)
          loss_str = "Loss function value = %1.12f" % eval_results["loss"]

          print(step_str)
          print(duration_str)
          print(loss_str)
          output_array.extend([step_str, duration_str, loss_str])

          # Save results to file
          with open(os.path.join(FLAGS.model_dir, "benchmark.txt"), "w+") as f:
              f.write('\n'.join(output_array))
